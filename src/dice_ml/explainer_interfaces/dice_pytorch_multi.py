...
class DicePyTorchMulti(ExplainerBase):
    ...
    def __init__(self, data_interface, model_interface):
        ...
        print("** dice_pytorch_multi")

    def generate_counterfactuals(self, query_instance, total_CFs, desired_class=1, proximity_weight=0.5,
        ...
        # Modified: set the desired_class, which will be the target class for the generated CF example
    ...
    def get_model_output(self, input_instance):
        ...
        # Modified: return the probability for the desired_class
        return self.model.get_output(input_instance)[self.desired_class:(self.desired_class+1)]
    ...
    # Modified: added a function to predict the class, returns argmax of the output
    def predict_class(self, input_instance):
        if not torch.is_tensor(input_instance):
            input_instance = torch.tensor(input_instance).float()
        pred = self.model.get_output(input_instance).data.numpy()
        pred = np.argmax(pred)
    ...
    def predict_fn_for_sparsity(self, input_instance):
        ...
        # Modified: return the scores of all classes
        return self.model.get_output(torch.tensor(input_instance).float()).data.numpy()
    ...
    # Modified: since the categorical features are binary (in CARLA datasets), and we have applied one-hot encoding
    # with drop_first=True, so, there is no need for this regularization loss.
    ...
    def compute_loss(self):
        ...
        # Modified: removed regularization_loss
        ...
        for n in range(self.total_CFs):
            for i in range(len(self.minx[0])):
                if i in self.feat_to_vary_idxs:
                    ...
                    # Modified: because of an error
                    self.cfs[n].data[i] = torch.tensor(query_instance[i]).float()
    ...
    def round_off_cfs(self, assign=False):
        ...
        for index, tcf in enumerate(self.cfs):
            ...
            for v in self.encoded_categorical_feature_indexes:
                # Modified: this part is changed to handle drop_first=True
                if cf[v[0]] >= 0.5:
                    cf[v[0]] = 1.0
                else:
                    cf[v[0]] = 0.0
    ...
    def stop_loop(self, itr, loss_diff):
        ...
        if loss_diff <= self.loss_diff_thres:
            ...
            if self.loss_converge_iter < self.loss_converge_maxiter:
                ...
                # Modified: included the class in the condition
                temp_cfs = self.round_off_cfs(assign=False)
                test_preds = [self.predict_fn(cf)[0] for cf in temp_cfs]
                test_class = [self.predict_class(cf) for cf in temp_cfs]

                if all(i == self.desired_class for i in test_class) \
                        and all(i >= self.stopping_threshold for i in test_preds):
                    self.converged = True
                    return True
                else:
                    return False
    ...
    def find_counterfactuals(self, query_instance, desired_class, optimizer, learning_rate, min_iter,
        ...
        # Modified: set the desired_class. Note that target_cf_class is 1 here, as in their implementation, this value
        # is used as the probability we want to get close to
        self.desired_class = desired_class
        self.target_cf_class = torch.tensor(1).float()
        ...
        # Modified: the following is not required for multi-class case
        ...
        self.best_backup_cfs_class = [0] * max(self.total_CFs, loop_find_CFs)  # Modified: added to store class
        ...
        for loop_ix in range(loop_find_CFs):
            ...
            while self.stop_loop(iterations, loss_diff) is False:
                ...
                # Modified: check for the predicted class
                temp_cfs_stored = self.round_off_cfs(assign=False)
                test_preds_stored = [self.predict_fn(cf) for cf in temp_cfs_stored]
                test_class_stored = [self.predict_class(cf) for cf in temp_cfs_stored]

                if all(i == self.desired_class for i in test_class_stored) and all(i >= self.stopping_threshold for i in test_preds_stored):
                    ...
                    if avg_preds_dist < self.min_dist_from_threshold[loop_ix]:
                        ...
                        for ix in range(self.total_CFs):
                            ...
                            self.best_backup_cfs_class[loop_ix + ix] = copy.deepcopy(test_class_stored[ix])
        ...
        self.cfs_class = [self.predict_class(cfs) for cfs in self.final_cfs]

        # update final_cfs from backed up CFs if valid CFs are not found
        if any(i != self.desired_class for i in self.cfs_class) \
                or any(i[0] < self.stopping_threshold for i in self.cfs_preds):
            for loop_ix in range(loop_find_CFs):
                if self.min_dist_from_threshold[loop_ix] != 100:
                    for ix in range(self.total_CFs):
                        ...
                        self.cfs_class[loop_ix + ix] = copy.deepcopy(self.best_backup_cfs_class[loop_ix + ix])
        ...
        for tix in range(max(loop_find_CFs, self.total_CFs)):
            ...
            self.cfs_class[tix] = np.array([self.cfs_class[tix]], dtype=np.float32)
            ...
            if isinstance(self.best_backup_cfs[0], np.ndarray):  # checking if CFs are backed
                ...
                self.best_backup_cfs_class[tix] = np.array([self.best_backup_cfs_class[tix]], dtype=np.float32)
        ...
        # Modified: check for the predicted class
        if all(i == self.desired_class for i in self.cfs_class) \
                and all(i >= self.stopping_threshold for i in self.cfs_preds):
            ...
            for cf_ix, pred in enumerate(self.cfs_preds):
                if self.cfs_class[cf_ix] == self.desired_class and pred[0][0] > self.stopping_threshold:
    ...
    # Modified: override the is_cf_valid function from the ExplainerBase to include maximum class
    def is_cf_valid(self, model_score):
        """Check if a cf belongs to the target class or target range.
        """
        validity = True if (np.argmax(model_score) == self.desired_class and
                            model_score[self.desired_class] >= self.stopping_threshold) else False
