    ...
    def is_cf_valid(self, model_score):
        ...
        if self.model.model_type == ModelTypes.Classifier:
            ...
            if self.num_output_nodes == 1:  # for tensorflow/pytorch models
                ...
                return validity
            if self.num_output_nodes == 2:  # binary
                # Modified: to handle an error caused by our implementation of the classifier. We
                # consider n_class output + softmax, but, dice considers one output+sigmoid. The error was caused
