...
class PublicData(_BaseData):
    ...
    def check_mad_validity(self, feature_weights):
        ...
        if feature_weights == "inverse_mad":
            # modified: set display_warnings=False
            self.get_valid_mads(display_warnings=False, return_mads=False)
    ...
    def one_hot_encode_data(self, data):
        ...
        # Modified: changed drop_first to True. This is to be consistent with CARLA where they apply one-hot encoding
        # with drop if binary option
    ...
    def from_dummies(self, data, prefix_sep='_'):
        ...
        for feat in self.categorical_feature_names:
            ...
            # Modified: this part is changed since we are applying drop_first=True
            out[feat] = pd.Categorical([self.data_df[feat].cat.categories[int(data[cols].values[0, 0])]])
