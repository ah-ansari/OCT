...
def graph_search(
    ...
    # Modified: This is a bug in their code, that they do not change index after a subsample of the data is selected in the
    # line above. I found this bug from an error when the index is larger than the subsample size.
    new_index = list(data.index.values).index(index)
    ...
    # Modified: added to avoid error for the case that all the features are mutable
    ...
    if p_norm == 1:
        c_dist = np.abs((data.values[new_index] - candidate_counterfactual_star)).sum(
            axis=1
        )
    elif p_norm == 2:
        c_dist = np.square((data.values[new_index] - candidate_counterfactuals)).sum(axis=1)
    else:
...
def build_graph(
    ...
    # Modified: we consider all the features to be mutable, so the immutable_constraint_matrix is not defined,
    # the following is commented to avoid error
