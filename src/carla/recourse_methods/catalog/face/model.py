...
class Face(RecourseMethod):
    ...
    def get_counterfactuals(self, factuals: pd.DataFrame) -> pd.DataFrame:
        ...
        # Modified: Modified: 1. to use df_train instead of df, 2. to use clean data
        df = self._mlmodel.data.df_train.copy()

        import tools

        x_train = df[self._mlmodel.train_order].values
        ood_oracle = tools.create_ood_oracle(x_train)
        y_ood = ood_oracle.predict(x_train)
        df = df.iloc[y_ood == 1]
        # End
