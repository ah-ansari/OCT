...
class FeatureTweak(RecourseMethod):
    ...
    # Modified: added class labels as input to handle the multi-class case
    def __init__(
        self,
        mlmodel: MLModelCatalog,
    ...
    def feature_tweaking(self, x: np.ndarray, class_labels: List[int], cf_label: int):
        ...
        def predict(classifier, x):
            # Modified: added MLModel as one of the instances in the first if
            if isinstance(
                classifier,
                (sklearn.tree.DecisionTreeClassifier, MLModelCatalog, MLModel),
            ):
        ...
        for tree in self.model.tree_iterator:  # loop over individual trees
            ...
            if (
                ...
                for key in paths_info:
                    ...
                    # Modified: I believe this is a bug, the model should check for the model to assign cf_label
                    # not the tree
                    if (
                        predict(self.model, es_instance) == cf_label
    ...
    def get_counterfactuals(self, factuals: pd.DataFrame):
        ...
        # Modified: added class_labels as input of the model to handle the multi-class case
        # class_labels = [0, 1]
        ...
        for i, row in instances.iterrows():
            ...
            counterfactual = self.feature_tweaking(
                row.to_numpy(), self.class_labels, cf_label
            )
