...
class CCHVAE(RecourseMethod):
    ...
    def __init__(self, mlmodel: MLModel, hyperparams: Dict = None) -> None:
        ...
        # Modified: added sklearn
        supported_backends = ["pytorch", "sklearn", "xgboost"]
    ...
    def _counterfactual_search(
        ...
        # Modified: no need for instance label as target class is 1
        ...
        while count <= self._max_iter or len(candidate_dist) <= 0:
            ...
            # Modified: no need for changing the input to torch tensor as it is handled in the MyOwnModel class
            y_candidate = np.argmax(
                self._mlmodel.predict_proba(x_ce),
                axis=1,
            )

            # Modified: select valid CF examples as ones with class 1
