...
def check_counterfactuals(
    ...
    # Modified: the following line produces error when the input is set to nan by the model
    # df_cfs[mlmodel.data.target] = np.argmax(mlmodel.predict_proba(df_cfs), axis=1)
    df_cfs.loc[~df_cfs.isnull().any(1), mlmodel.data.target] = np.argmax(
        mlmodel.predict_proba(df_cfs[~df_cfs.isnull().any(1)]), axis=1)

    # Change all wrong counterfactuals to nan
    # Modified: set those with pred != 1 as nan
    df_cfs.loc[df_cfs[mlmodel.data.target] != 1, :] = np.nan
